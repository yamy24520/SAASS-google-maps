...
import {
  ...
  Image as ImageIcon, Camera, MapPin, Star, Globe, LayoutGrid, Eye, EyeOff, AlertTriangle,
} from "lucide-react"
...
export default function PersonnalisationPage() {
  ...
  const dirty = state.pageTheme !== saved.pageTheme
    ...
    || state.pageCoverHeight !== saved.pageCoverHeight
    ...
    Promise.all([
      ...
      if (b) {
        const initial: PreviewState = {
          ...
          pageCoverHeight:  b.pageCoverHeight ?? null,
    ...
    try {
      ...
      const textRes = await fetch(`/api/settings${bizParam}`, {
        ...
        body: JSON.stringify({
          ...
          pageCoverHeight:  state.pageCoverHeight,
        }),
      })
    ...
    <div className="fixed inset-0 lg:left-64 top-16 flex flex-col bg-slate-100 z-10">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 flex-shrink-0 bg-white border-r border-slate-200 overflow-y-auto">
          ...
          {/* ─ Banniere / Cover ─ */}
          <div className={`rounded-2xl border-2 p-4 space-y-3 ${state.pageCoverDataUrl ? "border-slate-200 bg-white" : "border-amber-300 bg-amber-50"}`}>
            <div className="flex items-center gap-2">
              <Camera className="w-4 h-4 text-slate-600 flex-shrink-0" />
              <p className="text-sm font-semibold text-slate-800 flex-1">Bannière de couverture</p>
              {!state.pageCoverDataUrl && (
                <span className="flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-200 px-2 py-0.5 rounded-full">
                  <AlertTriangle className="w-3 h-3" /> Manquante
                </span>
              )}
            </div>
            {!state.pageCoverDataUrl && (
              <p className="text-xs text-amber-700 bg-amber-100 rounded-lg px-3 py-2">
                La bannière est affichée en grand sur votre page réputation et en haut du formulaire de réservation. Elle donne une première impression forte — ajoutez-en une.
              </p>
            )}
            {state.pageCoverDataUrl ? (
              <div className="space-y-3">
                <div className="relative rounded-xl overflow-hidden border border-slate-200" style={{ height: `${state.pageCoverHeight ?? 180}px` }}>
                  <img src={state.pageCoverDataUrl} className="w-full h-full object-cover" alt="cover" />
                </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <p className="text-xs font-medium text-slate-500">Hauteur de la bannière</p>
                    <span className="text-xs font-semibold text-slate-700">{state.pageCoverHeight ?? 180}px</span>
                  </div>
                  <input
                    type="range" min={80} max={400} step={10}
                    value={state.pageCoverHeight ?? 180}
                    onChange={e => update("pageCoverHeight", Number(e.target.value))}
                    className="w-full accent-sky-500"
                  />
                  <div className="flex justify-between text-[10px] text-slate-400 mt-0.5"><span>80px</span><span>400px</span></div>
                </div>
              </div>
            ) : (
              <label className="flex flex-col items-center gap-3 w-full py-10 rounded-xl border-2 border-dashed border-amber-300 hover:border-amber-400 hover:bg-amber-100/50 cursor-pointer transition-all">
                <div className="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center">
                  <Camera className="w-6 h-6 text-amber-500" />
                </div>
                <div className="text-center">
                  <p className="text-sm font-semibold text-slate-700">Ajouter une bannière</p>
                  <p className="text-xs text-slate-400 mt-0.5">Recommandé : 1200 × 400 px</p>
                </div>
                <input type="file" accept="image/*" onChange={handleImageUpload("pageCoverDataUrl")} className="hidden" />
              </label>
            )}
